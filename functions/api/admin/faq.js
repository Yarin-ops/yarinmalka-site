...
const FILE_PATH = 'assets/data/faq.json';
const BRANCH = 'main';
...
export async function onRequestGet({ env }) {
  try {
    ...
    return new Response(JSON.stringify({ faqs: content.faqs, sha }), { headers: { 'content-type': 'application/json' } });
  } catch (e) {
...
export async function onRequestPut({ request, env }) {
  try {
    ...
    if (!Array.isArray(body.faqs)) {
      return new Response(JSON.stringify({ error: 'faqs must be an array' }), { status: 400, headers: { 'content-type': 'application/json' } });
    }
    const { sha } = await getFile(env);
    await putFile(env, { faqs: body.faqs }, sha, body.message || 'Update FAQ via admin');
