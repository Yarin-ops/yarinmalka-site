...
export async function onRequestGet({ env }) {
  try {
    ...
    return new Response(JSON.stringify({ settings: content, sha }), { headers: { 'content-type': 'application/json' } });
  } catch (e) {
...
export async function onRequestPut({ request, env }) {
  try {
    ...
    if (!body.settings) {
      return new Response(JSON.stringify({ error: 'settings missing' }), { status: 400, headers: { 'content-type': 'application/json' } });
    }
    const { sha } = await getFile(env);
    await putFile(env, body.settings, sha, body.message || 'Update settings via admin');
