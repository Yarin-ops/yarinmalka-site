...
const FILE_PATH = 'assets/data/testimonials.json';
const BRANCH = 'main';
...
export async function onRequestGet({ env }) {
  try {
    ...
    return new Response(JSON.stringify({ testimonials: content.testimonials, sha }), {
...
export async function onRequestPut({ request, env }) {
  try {
    ...
    if (!Array.isArray(body.testimonials)) {
      return new Response(JSON.stringify({ error: 'testimonials must be an array' }), { status: 400 });
    }
    const { sha } = await getFile(env);
    await putFile(env, { testimonials: body.testimonials }, sha, body.message || 'Update testimonials via admin');
